...
int syscall_get_dir(char *path) { return msyscall(SYS_get_dir, path); }

int syscall_ch_dir(const char *path) { return msyscall(SYS_ch_dir, path); }

int syscall_set_args(const char *name, const char *value, int exported,
                     int readonly) {
    return msyscall(SYS_set_args, name, value, exported, readonly);
}
