...
static void __attribute__((noreturn)) cow_entry(struct Trapframe *tf) {
    ...
    // 检查页面是否有效
    // if (!(vpt[VPN(va)] & PTE_V)) {
    //     user_panic("cow_entry: page not valid at %08x", va);
    // }

    // perm = PTE_FLAGS(vpt[VPN(va)]);
    // if (!(perm & PTE_COW)) {
    //     user_panic("cow_entry: not a COW page at %08x, perm=%08x", va, perm);
    // }
    ...
    syscall_mem_map(0, (void *)UCOW, 0, va, perm);
...
int fork(void) {
    ...
    for (i = 0; i < USTACKTOP / PAGE_SIZE; i++) {
        if (vpt[i] & PTE_V) {
            duppage(child, i);
        }
    }
