...
void page_init(void) {
    ...
    int used_pages = (freemem - ULIM) / PAGE_SIZE;
    for (int i = 0; i < used_pages; i++) {
        (pages[i]).pp_ref = 1;
    }
    /* Step 4: Mark the other memory as free. */
    for (int i = used_pages; i < npage; i++) {
        pages[i].pp_ref = 0;
...
//	printk("!!!%d", size);
	...
	LIST_FOREACH(mb, &mblock_list, mb_link) {
		...
		if (mb->size >= size + 32) {
			...
			struct MBlock *new = mb->data + size;
			new->size = mb->size - size - 24;
			new->ptr = NULL;
			new->free = 1;
			LIST_INSERT_AFTER(mb, new, mb_link);
	//		printk("%xwwwwwwwww\n", (void*)mb);
			return mb->data;
...
void free(void *p) {
	...
	struct MBlock * mb = (void*)p - MBLOCK_SIZE;
